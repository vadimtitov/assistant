...
INTERVALS = {'north east': (22.5, 67.5),
             'east':       (67.5, 112.5),
...
class WeatherRecord(object):
    def __init__(self, r):
        ...
        "self.sunrise = None", #dt.utcfromtimestamp(r['sys']['sunrise']).strftime('%H:%M')
        "self.sunset = None", #dt.utcfromtimestamp(r['sys']['sunset']).strftime('%H:%M')
...
class Weather(WeatherRecord):
    ...
    def __current(self):
        record = requests.get(f'http://api.openweathermap.org/data/2.5/weather?{self.params}').json()
        WeatherRecord.__init__(self, record)

    def __forecast(self):
        r = requests.get(f'http://api.openweathermap.org/data/2.5/forecast?{self.params}').json()
        records = r['list']
...
"""
    ...
    def summary(self):
        if self.temp_max - self.temp_min >= 3:
            summary = f"""You can observe {self.conditions[0]} in {self.city} right now,
    ...
    def fullSummary(self):
        if self.temp_max - self.temp_min >= 3:
            opts = f"""Current temperature in {self.city} is {self.temp} degrees,
                       you can observe {self.conditions[0]}.
                       The {self.wind_dir}ern wind's speed is {self.wind_speed} meters per second.
                       Humidity is {self.humidity} %"""
        else:
            opts = f"""The temperature in {self.city} varies between
    ...
    def min(self, days_ahead = 0, daily = True):
        if daily:
            ...
            return min([temp.temp for temp in self.forecasts[days_ahead] if temp])
...
"""
        result = \
        [["Temperature", f"{self.temp} °C, from {self.temp_min} °C to {self.temp_max} °C"],
         ["Conditions", f"{conditions}"],
         ["Wind", f"{self.wind_speed} m/s, {self.wind_dir}"],
         ["Pressure", f"{self.pressure} mmHg"],
         ["Humidity", f"{self.humidity} %"],
         ["Sunrise", f"{self.sunrise}"],
         ["Sunset", f"{self.sunset}"]]
        return tabulate(result, tablefmt = "presto")
"""
